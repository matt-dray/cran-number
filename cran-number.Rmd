...
Create a matrix of all the relationships. This takes a while to run given the amount of data.

```{r}
tictoc::tic()

edge_list <- cran_authors_df %>%
  map(combn, m = 2) %>%
  do.call("cbind", .) %>%
  t() %>%
  dplyr::as_data_frame() %>%
...
Turn the edge list into a tidygraph object.
